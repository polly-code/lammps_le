...
FixExUnload::FixExUnload(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  partner(NULL), finalpartner(NULL), distsq(NULL), probability(NULL),
  broken(NULL), copy(NULL), random(NULL)
{
  if (narg < 6) error->all(FLERR,"Illegal fix ex_unload command");

  MPI_Comm_rank(world,&me);
  MPI_Comm_size(world,&nprocs);

  nevery = utils::inumeric(FLERR,arg[3],false,lmp);
  if (nevery <= 0) error->all(FLERR,"Illegal fix ex_unload command");
  ...
  if (btype < 1 || btype > atom->nbondtypes)
    error->all(FLERR,"Invalid bond type in fix ex_unload command");
  if (cutoff < 0.0) error->all(FLERR,"Illegal fix ex_unload command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"prob") == 0) {
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix ex_unload command");
      fraction = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      seed = utils::inumeric(FLERR,arg[iarg+2],false,lmp);
      if (fraction < 0.0 || fraction > 1.0)
        error->all(FLERR,"Illegal fix ex_unload command");
      if (seed <= 0) error->all(FLERR,"Illegal fix ex_unload command");
      iarg += 3;
    } else error->all(FLERR,"Illegal fix ex_unload command");
  }

  // error check

  if (atom->molecular != 1)
    error->all(FLERR,"Cannot use fix ex_unload with non-molecular systems");
...
void FixExUnload::init()
{
  ...
  if (force->improper) {
    if (force->improper_match("class2") || force->improper_match("ring"))
      error->all(FLERR,"Cannot yet use fix ex_unload with this "
                 "improper style");
  }
...
void FixExUnload::post_integrate()
{
  int i,j,k,m,n,i1,i2,n1,n3,type;
  double delx,dely,delz,rsq;
  tagint *slist;

  if ((update->ntimestep) % nevery - 2) return; //+2 to avoid problems with bond create and extrusion
  ...
  if (atom->nmax > nmax) {
    ...
    memory->destroy(distsq);
    nmax = atom->nmax;
    memory->create(partner,nmax,"ex_unload:partner");
    memory->create(finalpartner,nmax,"ex_unload:finalpartner");
    memory->create(distsq,nmax,"ex_unload:distsq");
    probability = distsq;
  }
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    i2 = bondlist[n][1];
    type = bondlist[n][2];
    //printf("i1 and i2: local %d, %d global %d, %d\n", i1, i2, tag[i1], tag[i2]);
  ...
  for (i = 0; i < nall; i++) {
    ...
    if (j < 0 || tag[i] < tag[j]) {
      if (nbreak == maxbreak) {
        maxbreak += DELTA;
        memory->grow(broken,maxbreak,2,"ex_unload:broken");
      }
...
void FixExUnload::check_ghosts()
{
  ...
  if (flagall)
    error->all(FLERR,"Fix ex_unload needs ghost atoms from further away");
...
void FixExUnload::update_topology()
{
...
void FixExUnload::rebuild_special_one(int m)
{
...
void FixExUnload::break_angles(int m, tagint id1, tagint id2)
{
  int j,found;
...
void FixExUnload::break_dihedrals(int m, tagint id1, tagint id2)
{
  int j,found;
...
void FixExUnload::break_impropers(int m, tagint id1, tagint id2)
{
  int j,found;
...
int FixExUnload::dedup(int nstart, int nstop, tagint *copy)
{
  int i;
...
void FixExUnload::post_integrate_respa(int ilevel, int /*iloop*/)
{
...
int FixExUnload::pack_forward_comm(int n, int *list, double *buf,
...
void FixExUnload::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,ns,last;
...
int FixExUnload::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixExUnload::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixExUnload::print_copy(const char *str, tagint m,
...
double FixExUnload::compute_vector(int n)
{
...
double FixExUnload::memory_usage()
{
  int nmax = atom->nmax;
