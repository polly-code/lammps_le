                                                          ...
                                                          probability(nullptr), created(nullptr), copy(nullptr), random(nullptr), list(nullptr)
{
  ...
  if (iatomtype == jatomtype &&
    ...
    error->all(FLERR,
               "Inconsistent iparam/jparam values in fix ex_load command");
...
void FixExLoad::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
int FixExLoad::dedup(int nstart, int nstop, tagint *copy)
{
  int i;
...
void FixExLoad::copy_arrays(int i, int j, int /*delflag*/)
{
...
int FixExLoad::pack_exchange(int i, double *buf)
{
  buf[0] = bondcount[i];
  return 1;
}
